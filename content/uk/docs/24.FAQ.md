...
::questions
  ...
  ::qa{question="Якщо мій додаток використовує інший стек і не базується на React.js, Vue.js чи Angular?" id="different-stack"}
    Якщо ви використовуєте фреймворк, відмінний від React.js, Vue.js або Angular (для якого Rete.js надає рендерінг плагін), ви можете скористатися [плагіном рендерінгу React.js](/docs/guides/rendering/plugin) для візуалізації вузлів та інших елементів редактора.
