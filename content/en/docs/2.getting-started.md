...
```bash
npm i rete@next rete-area-plugin@next rete-connection-plugin@next rete-render-utils@next rete-react-plugin@next react react-dom
```
...
<script src="https://cdn.jsdelivr.net/npm/rete-react-plugin@next/react-render-plugin.min.js"></script>
```
