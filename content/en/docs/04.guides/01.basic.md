...
If your application allows node selection, users may want to view selected nodes without any visual obstructions. To facilitate this, additional extensions are provided that automatically bring the selected node to the front.

```ts
AreaExtensions.simpleNodesOrder(area);
```

`simpleNodesOrder` reorders elements in the DOM. If your nodes contain click handlers and you need click-safe ordering, use the z-index based alternative:
