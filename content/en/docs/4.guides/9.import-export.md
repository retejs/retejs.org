...
Additionally, importing or exporting inputs and outputs may not always be necessary if they are static and we know the node type. In such cases, we can simply save the node name and relevant data, which can be used to create instances of nodes with pre-defined ports. Check out [the example](https://codesandbox.io/s/rete-js-2-3d-configurator-43o7ex?file=/src/node-editor/import.js) with implementation of this approach
